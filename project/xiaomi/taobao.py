...
# 登录的网站
URLLogin = 'https://login.taobao.com/'

# 预约的网站
URLOrder = "http://a.hd.xiaomi.com/register/book/a/35"
IDUserName = 'TPL_username_1'
IDPwd = "TPL_password_1"
class MiWeb():
...
#             WebDriverWait(dr, 5).until_not(lambda dr: dr.find_element_by_id('miniLogin_pwd').is_displayed())
            ...
            WebDriverWait(dr, 5).until(lambda dr: dr.find_element_by_id("J_SafeLoginCheck").is_displayed())
            checkbox = dr.find_element_by_id('J_SafeLoginCheck')
            checkbox.click()

            WebDriverWait(dr, 5).until(lambda dr: dr.find_element_by_id(IDPwd).is_displayed())
...
user = User.WebUser()
user.uid = "13423997110"
user.pwd = "agz050772447"
MiWeb(user, dr).login()